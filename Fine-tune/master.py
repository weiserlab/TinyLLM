...
parser.add_argument("-d", "--dataset_type", help="Type of dataset: breathe, local, gesture", required=True)
parser.add_argument("-m", "--model_dir", help="Path to the model", required=True)
parser.add_argument("-rn", "--random_number", help="Plot log file number", required=False)
...
lora_r = 128# r is the rank of the LoRA attention matrix, default is 16 
lora_alpha = 128 # alpha is the scaling factor for the LoRA attention matrix, default is 64
lora_dropout = 0.25 # Dropout probability for LoRA layers
bias = "lora_only"
task_type = "CAUSAL_LM"
